...
type Movie struct {
	...
	Runtime   MovieRuntime `json:"runtime,omitempty,string"`
...
func (m MovieModel) Get(id int64) (*Movie, error) {
	...
	// NOTE(Farid): We can start the server with -db-max-open-conns=1 to test if
	// the application handles the timeouts gracefully
	// query := `
	...
	query := `
	SELECT pg_sleep(1), id, created_at, title, year, runtime, genres, version
	FROM movies
	WHERE id = $1`
...
func (m MovieModel) Update(movie *Movie) error {
	// TODO(Farid): Update the database so it can accept 'version' as uuid type
	// query := `
	// UPDATE movies
