...
# runing timedatectl list-timezones.
TIMEZONE=America/Mexico_City

# Set the name of the new user to create
USERNAME=greenlight

# Prompt to enter a passwrod for the PostgreSQL greenlight user (rather than hardcoding
# a password for this script).
read -p "Enter password for greenlight DB user: " DB_PASSWORD

# Force all output to be presented in en_US for the duration of this script. This aoivds
# any "setting locale failed" errors while this cript is running, before we have
...
# Enalble the "universe" repository
add-apt-repository --yes universe

# Update all software packages. Using the --force-confnew flag means taht configuration
...
rsync --archive --chown=${USERNAME}:${USERNAME} /root/.sh /home/${USERNAME}
...
# Install the migarte CLI tool.
curl -L https://github.com/golang-migrate/migrate/releases/download/v4.14.1/migrate.linux-amd64.tar.gz | tar xvz
mv migrate.linux-amd64 /usr/local/bin/migrate

# Install PostgreSQL
apt --yes install postygresql
